 ...
 * @param {boolean} [prior=false] - package has priority or not
 * @returns {Buffer}
 */
TcpProtocol.prototype.serialize = function (id, code, data, prior) {
    ...
    buffer.writeUInt8(prior ? 1 : 0, 6);

    crc = 0x00 ^ id ^ code ^ length ^ (prior ? 1 : 0);
 ...
 *
 * @param {Buffer} [data]
 * @returns {{valid: boolean}|{valid: boolean, id: int, code: int, length: int, prior: boolean, data: Buffer}}
 */
TcpProtocol.prototype.unserialize = function (data) {
    var i, length, id, code, prior, crc, offset, valid, response;
